...
from core.utils import iae, ise, itae, itse, sin_angle, cos_angle, dropna, clip, random_uniform, save_results, seconds_to_hms
from core.physics import VehiclePhysics

def calculatePID(params, error, prevError, sumError):
    kp, ki, kd = params
    PID = ((kp * error) + (ki * sumError) + (kd * (error - prevError)))
    return PID
...
def lean_simulate_system(params, obj_function, return_history=False, sim_time=60, theta_angle=0, vehicle_mass=1500, rolling_resistance=0.06):
