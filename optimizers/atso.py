...
class ArchivedTunaSwarmOptimization:
    def __init__(self, fitness_function, min_params, max_params,
                 pop_size=100, max_iter=300, a=0.7,z=0.05, maximize=True, progress_callback=None, archive_size=4):
        ...
        # External archive: keeps the best tunas found so far.
    ...
    def __update_archive(self, tunas, fitness):
        if self.archive_positions is not None:
            tunas = xp.concatenate([tunas, self.archive_positions], axis=0)
            fitness = xp.concatenate([fitness, self.archive_fitness], axis=0)

        tunas, fitness = self.__sort_by_fitness(tunas, fitness)
        keep_archive = min(self.archive_size, int(tunas.shape[0]))

        self.archive_positions = tunas[:keep_archive].copy()
        self.archive_fitness = fitness[:keep_archive].copy()

        keep_pop = min(self.pop_size, int(tunas.shape[0]))
        return tunas[:keep_pop].copy(), fitness[:keep_pop].copy()
    ...
    def run(self, verbose=True):
        ...
        for t in iterator:
            ...
            tunas_old, fitness = self.__update_archive(tunas_old, fitness)
            ...
            tunas_old, fitness = self.__update_archive(tunas_new, fitness)

            self.history["best_fitness"].append(float(to_cpu(self.archive_fitness[0])))
            self.history["best_position"].append(to_cpu(self.archive_positions[0]))
        ...
        return to_cpu(self.archive_positions[0]), float(to_cpu(self.archive_fitness[0]))
...
def run_atso(func,
            ...
            seed=None,
            archive_size=4):
    model = ArchivedTunaSwarmOptimization(
