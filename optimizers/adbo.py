...
class ArchivedDungBeetleOptimizer:
    ...
    def _sync_archive(self):
        ...
        items = [(agent.position.copy(), agent.value) for agent in self.beetles]
        items.extend([
            (pos.copy(), fit)
    ...
    def _inject_archive(self):
        ...
        ordered = list(range(len(self.beetles)))
        ordered.sort(
            key=lambda idx: self._fitness_as_float(self.beetles[idx].value),
            reverse=not self.maximize
        )

        for rank, idx in enumerate(ordered[:len(self.archive_positions)]):
            self.beetles[idx].position = self.archive_positions[rank].copy()
            self.beetles[idx].value = self.archive_fitness[rank]
            if hasattr(self.beetles[idx], "previous_position"):
                self.beetles[idx].previous_position = self.archive_positions[rank].copy()
