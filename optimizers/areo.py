...
class ArchivedRippleEvolutionOptimizer:
    def __init__(self, fitness_function, min_params, max_params, maximize=False,
                 ...
                 progress_callback=None, archive_size=4):
    ...
    def _sync_archive(self):
        ...
        items = [(agent.position.copy(), agent.value) for agent in self.agents]
        items.extend([
            (pos.copy(), fit)
    ...
    def _inject_archive(self):
        ...
        ordered = list(range(len(self.agents)))
        ordered.sort(
            key=lambda idx: self._fitness_as_float(self.agents[idx].value),
            reverse=not self.maximize
        )

        for rank, idx in enumerate(ordered[:len(self.archive_positions)]):
            self.agents[idx].position = self.archive_positions[rank].copy()
            self.agents[idx].value = self.archive_fitness[rank]
            if hasattr(self.agents[idx], "previous_position"):
                self.agents[idx].previous_position = self.archive_positions[rank].copy()
